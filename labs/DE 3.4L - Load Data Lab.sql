...
-- MAGIC %run "/Workspace/Users/euheniy.khvoinitski@gmail.com/Databricks DE Course/DE 3 - Delta Lake/Includes/Classroom-Setup-03.6L"
...
CREATE
OR REPLACE TABLE hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw (
  key BINARY
  COMMENT "user_id is used as the key. Unique alphanumeric field that corresponds to session/cookie",
  offset LONG
  COMMENT "Unique value, monotonically increasing for each partition",
  partition INTEGER
  COMMENT "The partition number",
  timestamp LONG
  COMMENT "Milliseconds since epoch, represents the time at which the producer appends a record to a partition",
  topic STRING
  COMMENT "Only those records from the clickstream topic are included here",
  value BINARY
  COMMENT "This is the full data payload, sent as JSON"
)
COMMENT "Each file contains all records consumed during a 5-second interval, stored with the full Kafka schema as a multiple-record JSON file";

-- COMMAND ----------

DESCRIBE EXTENDED hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw;

-- COMMAND ----------

DESCRIBE HISTORY hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw;
...
SELECT
  *
FROM
  hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_json
LIMIT
  10;

-- COMMAND ----------

DESCRIBE EXTENDED hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_json;


-- COMMAND ----------

-- insert each record with a key, even already existing key (provided by offset inside partition)
-- merge to make indempotent
MERGE INTO
  hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw AS tgt
USING
  (
    SELECT * FROM hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_json
    -- Optional: Apply filtering to reduce the dataset
...
SELECT
  *
FROM
  hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw
LIMIT 10;
...
-- there are duplicates by payload with different offset, need to understand the nature of the duplicates
-- maybe possible that not all the properties are tracked
SELECT
  offset,
...
FROM
  hive_metastore.euheniy_khvoinitski_5nea_da_dewd.events_raw
WHERE
  key = CAST(unbase64('VUEwMDAwMDAxMDczNzMxMzc=') AS BINARY);
...
CREATE
OR REPLACE TABLE hive_metastore.euheniy_khvoinitski_5nea_da_dewd.item_lookup AS
SELECT
  *
FROM
...
SELECT
  *
FROM
  hive_metastore.euheniy_khvoinitski_5nea_da_dewd.item_lookup;
